...
-   数据可靠落盘，能够应对异常断电；
-   通过 Wi-Fi 与手机连接；
-   通过手机 Web 完成状态查看、开始 / 停止采集、日志查看与下载等操作；
-   盒子上电后自动进入可用状态；
-   USBcan、Wi-Fi 或采集服务出现异常时能够识别并恢复；
-   保持无屏、无键盘，不要求现场人员操作 Linux。
...
> **Linux 是地基，canboxd 是采集核心，uHTTPd/Web
> 是管理界面，手机是屏幕。**
...
生产 OS
的选择以降低软件开发、部署和长期维护复杂度为优先，不以极限降低硬件成本为目标。
...
uHTTPd 负责 HTTP 服务，Shell CGI / API
负责调用现有采集与系统管理能力；核心采集能力保持独立。
...
> **CAN BOX V0 是一个无屏、Linux 化的 CAN 智能采集网关：利用汽修店现有
> USBcan-II，通过车载 12V 供电、本地可靠存储、Wi-Fi 和手机 Web，实现
> Case 驱动的标准化 CAN 采集、管理、上传和后续远端诊断；第一版以 Pi 3B
> 快速验证完整闭环，生产 OS / 硬件平台以 USBcan 兼容性、可靠性和降低软件
> 开发部署维护复杂度为优先，再综合考虑成本。**
