...
source_last_edited_time: "2026-09-08T11:57:31.787Z"
...
│   ├─ DCDC维持低压母线
│   │
│   ├─ [TM3-017车型Evidence投射；以下方括号项不作为新增Node]
│   │   ├─ Condition：[同一连续高压充电运行态保持]
│   │   ├─ 操作者充电电流设置（外部输入）
│   │   │   ↓ 操作者改变设置后，车辆界面接受并保持新设置
│   │   ├─ 车辆接受并保持的充电电流设置（外部可观察状态）
│   │   │   ↓ [车辆内部处理区域未观测：不拆分、不命名]
│   │   ├─ → 实际电流 IAC_actual（引用下方既有Node）
│   │   │   ↓ 实际交流取电方向一致且可逆变化后进入新水平稳定状态
│   │   ├─ [Structural constraint：设置接受状态与实际响应状态不得合并]
│   │   └─ [Evidence / Boundary：见本控制树后的TM3-017正式增量说明]
│   │
│   ├─ 电池状态持续监测
│   │
│   ├─ 允许充电能力持续评估
...
【TM3-017 正式控制树最小增量（2026-09-15）】

适用对象与范围：`TESLA-M3-SOP5`、Round `S0030`、SOC约85%至86%、本次EVSE及现场条件；信息边界为`STRICT_NO_DBC_RAW_PAYLOAD_ONLY`。

正式准入：

- Node：操作者充电电流设置（外部输入）。E03与E06形成32→16→32 A的相反设置输入。
- Node：车辆接受并保持的充电电流设置（外部可观察状态）。E03-P01、E04-P01、E04-P03、E06-P01及E06-P02支持设置被界面接受并保持。
- Existing Node Evidence：既有`IAC_actual`获得本车方向一致且可逆的实际交流取电响应支持；页面实际电流呈32→16→32 A，页面功率呈7→4→7 kW。该Evidence不绑定任何raw候选，也不确认内部采样Signal。
- Existing State Evidence：既有⑭高压充电运行态在新设置下形成短时稳定结果；W1为70.000 s、W2为117.505 s、W3为102.522 s。
- Relationship：操作者设置→车辆接受并保持设置，强度为`SUPPORTED_DIRECT_RELATIONSHIP`。Event Clock仅为动作提示，不是触控完成时刻，因此不量化接受延迟。
- Relationship：已接受设置变化→实际交流取电方向一致且可逆变化，强度为`SUPPORTED_SYSTEM_LEVEL_CONTROL_RELATIONSHIP`。这是系统输入—输出层关系，中间必须保留未观测区域，不得表达为已知内部直连或1:1传递函数。
- Relationship：实际响应→新水平持续充电稳定结果，强度为`SUPPORTED_CONDITIONED_STATE_TRANSITION`。仅确认阶段顺序与本Round短时稳定，不确认精确收敛时刻。
- Structural constraint：设置接受状态与实际响应状态必须分离。E06-P01显示设置已经恢复至32 A而实际电流仍为21 A，E06-P02后实际电流才达到32 A；该证据不证明固定延迟或内部Node数量。
- Condition：上述Relationship仅在同一连续高压充电运行态下准入。本轮无已记录停充、重启或重连，页面持续显示正在充电；这不证明连接、锁止、Ready、许可或接触器各子状态。

未观测与不准入边界：

- 车辆接受设置与`IAC_actual`之间保留“车辆内部处理区域未观测”。不得把该区域拆分或命名为内部Target、能力、仲裁、请求或执行Node。
- 不建立车辆设置到既有`IAC_target`、`A₂`或`AEVSE`的车型级路径；本轮无法区分设置通过哪种内部机制生效。
- 页面可见可用上限未低于设置只保留为本轮实验范围；不得命名为`AEVSE`或`A₂`。
- 早期、中间及较晚匿名raw行为簇全部保留在TM3-017 Evidence附件，不进入正式控制树，不赋予物理量、控制Role、scale、unit、字段编码或ECU归属。
- 不准入raw簇之间的串行因果、复制、派生、同步或固定多byte关系；串行、共同潜变量并行及混合结构仍不可区分。
- 不外推至其他SOC、温度、EVSE、车辆、热限制、SOC限制、桩侧限流或异常保护工况。

Evidence引用：

- `output/TM3-017/TM3-017_Evidence_Assessment.md`
- `output/TM3-017/TM3-017_Candidate_Falsification_Behavioral_Review.md`
- `output/TM3-017/TM3-017_Cross_Round_Blind_Behavioral_Review.md`
- `output/TM3-017/TM3-017_Control_Structure_Layer_Review.md`
- `output/TM3-017/TM3-017_Formal_Control_Tree_Delta_Gate_Review.md`
- `output/TM3-017/evidence_assessment.csv`
- `output/TM3-017/tree_gate/node_gate.csv`
- `output/TM3-017/tree_gate/relationship_gate.csv`
- `output/TM3-017/tree_gate/condition_gate.csv`
...
![](https://prod-files-secure.s3.us-west-2.amazonaws.com/306c62d4-2d40-4184-a385-d164211324d2/3816510b-87de-4e71-85d6-1f45bad561ec/image.png?X-Amz-Algorithm=AWS4-HMAC-SHA256&X-Amz-Content-Sha256=UNSIGNED-PAYLOAD&X-Amz-Credential=ASIAZI2LB466YBNQ4FYV%2F20260911%2Fus-west-2%2Fs3%2Faws4_request&X-Amz-Date=20260911T032550Z&X-Amz-Expires=300&X-Amz-Security-Token=IQoJb3JpZ2luX2VjEMr%2F%2F%2F%2F%2F%2F%2F%2F%2F%2FwEaCXVzLXdlc3QtMiJHMEUCIEw3PNNasXdx5UvL0ilOZ85NL2umPoJsMCmolZ24RiUYAiEAnn1zRoQG%2F9bORdcdH31rANoQxM4Cw1OCBlCcze7tItcqiAQIk%2F%2F%2F%2F%2F%2F%2F%2F%2F%2F%2FARAAGgw2Mzc0MjMxODM4MDUiDJHWqgJeEp9vq%2F1fcSrcA5x%2FvsKMNDbXofUAYh8R1%2BhG9ifvzcsQVugYng8HmbzLPU9hAFPatsrHeNh9xiJ1UObMWyWneOyiTM7JY8pF595IHvkAGVWRogApyu5E%2FbybTMRLj%2FhIU3pZSrH0RTvMLA9%2FTy1lBpkeM7hkLAGgNN8OmrBmBiqiaWuswyOTwwDIkavfGsvo%2B4kKQAB9yhhLtatf8vrjJrrvluCpiNsLj9X0jFhhTVNHiI0oWIHPDIKlxBRdpsY%2BvyhTEqSnw8xR4oLF2mLCQAsqoAXnCBrshOlXN%2FPIIpUVhMH2ZgSowklqolhfc0G0ouNhHbi65A2nj0ReJW90BcfEQ%2Fz%2FOZSR0UymbwfVwBIs1jMfRSCU2%2FtWw1j3gagyv5FbhbC6ymq9yxkxOvH3PfMcH49qrGpp0hkZb5XbRwt6075T050WiKmE83HAfILJbdNAwOv7cJXwUi7NI4KiY9o3Jb%2BKAtcOKfeTlkLbn1nfKEm8qiW%2FIkt0BV9VDa8Evimfa1DeHkZ2EzVaZfa8Nw9hVSFLPcw5xBnRYKk12IfyTialiE02IvcPOc0DoXs0SxxA1TM%2FdXhzoN9l9kQptHp7qMapyt%2FDVSmbILvW99mLcg6Gp4fZHWIBEHN8kHPogoxqOddUMKjIjdUGOqUB%2BjuWMGLc0CV62fg5Hrj6p6R1WR7If05FHw1O1y2x2kxdecsERrycEJ7yE4rZ4p6TL0BoL%2BZys%2FbOqbOK8%2BswnUr8aWsCgU2DKm%2FcYxzZ2hUZRfME35EK2Mthsg86ALlwphJyQrGC7mc0zUGgbnGLJsMpxXjg41DmZd%2B9m6NAs%2BImwYHDN9i3RVtjYw2GKTm9gpxNvfGhNpf4l90ykumaS3k%2FdbR4&X-Amz-Signature=f05a6542df044885ad65148c455a88a30adfeb9f2ce9818bb4d03ac6c7a9c3de&X-Amz-SignedHeaders=host&x-amz-checksum-mode=ENABLED&x-id=GetObject)
① CC连接确认/控制确认
② CP通信
③ PE地
 ④ L1火线
