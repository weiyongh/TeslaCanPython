...
**Round 2 应确认到哪里：**伯恩应从单次 OFF→ON→OFF 中找到与 HVAC 总体状态同步变化的优先 CAN ID／raw bit，但不能确认具体树节点。

**Round 3 应确认到哪里：**伯恩应通过五个同条件会话确认这些候选具有重复性，并收敛为稳定的 CAN ID／字段组，而不是继续停留在零散 bit。

**两轮合起来能确认什么：**可以确认一批 ID／字段组属于“HVAC 总体状态相关观察”，但不能把它们进一步归入总开关设置、请求、许可、送风执行、制冷执行或反馈节点。

**伯恩是否已经尽力：**就“发现、复现和字段分组”而言基本尽力；若仍有可由现有两态数据恢复的完整字节轨迹、枚举关系或跨 ID 时序关系未分析，才属于 Round 3 分析未榨干。

**不能确认具体节点的主要原因：**Round 2 和 Round 3 都重复同一种总体 OFF/ON Diff，多个控制树节点被同时激励，因此数据本身缺乏节点间的可辨识性。

**这是分析问题还是采集问题：**主要是采集设计的辨识能力不足，而不是伯恩没有找到足够多的候选。

**如何用内部 DBC 判断责任：**如果 DBC 已知不同节点在现有数据中呈现可区分轨迹而伯恩遗漏，就是分析不足；如果已知节点在该 Diff 下也同步变化，就证明必须补充新条件。


**怎样收敛而不无限出计划：**下一轮只允许针对一个明确的“同步候选组为何无法分开”设计最小 Diff，并预先规定通过后应关闭哪些竞争解释，不能再做泛化的全总线候选搜索。


我们为什么采这组数据？它理论上应该让哪片控制树变得可辨识？AI 有没有把胶片里的信息洗出来？下一次让我拎电脑包之前，现有 Evidence 到底吃干净没有？